        ...
        if (splits.length > 1) {
          ...
          switch (newNodeName) {
            ...
            case "rowConstraints":
              ...
              break;
            default:
              String childrenGetter = switch (actualNode.getNodeName())
              {
                case "SplitPane" -> "getItems()";
                default -> "getChildren()";
              };
                Node source;
                String includeName, nodeName = childNode.getNodeName();
                switch (nodeName)
                {
                  case "fx:include":
                    source = childNode.getAttributes().getNamedItem("source");
                    includeName = source.getNodeValue();
                    includeName = includeName.substring(0, includeName.lastIndexOf('.'));
                    if (includeName != null && !includeName.isEmpty())
                    {
                      String includeNode = createInclude(includeName);
                      prefix = variableName + dot + childrenGetter;
                      declarationNode =
                              new DeclarationNode("{0}{1}({2});", new Object[]{prefix, ".add", includeNode});
                      childList.add(declarationNode);
                    }
                    break;
                  default:
                    fXNode = new FXNode(childNode, false);
                    if (fXNode.isDeclared())
                    {
                      attributeList.addAll(fXNode.getAttributeList());
                      childList.addAll(fXNode.getChildList());
